...
if ( !function_exists('reminiscence_lite_featured_category') ) :

    function reminiscence_lite_featured_category(){

        $reminiscence_lite_default = reminiscence_lite_get_default_theme_options();
        $enable_header_featured_category = get_theme_mod( 'enable_header_featured_category', $reminiscence_lite_default['enable_header_featured_category'] );

        if( $enable_header_featured_category ){

            $enable_header_featured_category_column = get_theme_mod( 'enable_header_featured_category_column', $reminiscence_lite_default['enable_header_featured_category_column'] );
                ...
                <div class="site-wrapper">
                    <div class="site-row">
                        ...
                        for( $i = 1; $i <= 7; $i++  ){
                            ...
                            $featured_category = get_theme_mod('reminiscence_lite_header_featured_category_cat_'.$i);
                                ...
                                <div class="site-column <?php echo $class_grid; ?> site-column-sm-12">
                                    <div class="wedevs-featured-categories data-bg <?php echo $grid_size; ?>" <?php if ($wedev_term_image) { ?> data-background="<?php echo esc_url($wedev_term_image); ?>" <?php } ?>>
                                        <a href="<?php echo esc_url($cat_link); ?>">
                                            ...
                                            <?php if( $count ){ ?>
                                                <span class="post-count"><?php echo esc_html( $count ).esc_html__(' Posts','reminiscence-lite'); ?></span>
                                            <?php } ?>
